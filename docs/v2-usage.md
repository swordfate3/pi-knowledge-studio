...
Exports are unique packages below `knowledge-studio-v2-exports/<output>/`, containing `document.md`, `document.html`, image assets and `sources.json`, `evidence.json`, `manifest.json`. JPEG/WebP originals are retained alongside separate PNG display renditions; provenance identifies both. Original bytes are not replaced by the display conversion. Keep the whole package together for offline viewing/sharing. Outputs include sensitive text and provenance; nothing is redacted.
...
- Collections live at `<cwd>/.pi/knowledge-studio-v2/<collection>/`, with SQLite catalog and content-addressed `blobs/`. The v2 base/collection directories and export base/output directories must be owned by the process user and **0700**. New directories are created safely; existing directories are not silently chmodded. If an owner-only error occurs, inspect the named directory and fix only that intended private directory (for example `chmod 700 <directory>`), not the entire workspace recursively.
