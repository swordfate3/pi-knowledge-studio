...
Approve each requested action in the trusted Pi confirmation UI. The standalone tools below keep their own confirmations. A normal interactive `ks_v2_generate` call is the exception: it presents one bundled confirmation for retrieval, optional hybrid/reranking, generation-model egress, and automatic export when an answer is produced. Search with no relevant hits reports an error rather than inventing an answer. `ks_v2_export` is deterministic **evidence compilation**, not model-generated prose; bundle/package UUIDs mean repeated exports are not byte-identical.
...
| `ks_v2_generate` | `collection`, `query`, `title`, `output`, optional `mode`, `limit`, `rerank` | One interactive bundled confirmation covers retrieval, optional hybrid/reranking, generation-model egress, and automatic export of a valid answer; headless hosts still need the underlying `search`, `generate`, `export`, `embedding` and `rerank` grants at their actual boundaries. Title: 1–1024 characters. |
...
inference, server modification, retry or fallback exists. Generation approval shows the profile/controls without secrets. In interactive `ks_v2_generate` this is part of the single bundled workflow confirmation; standalone tools and headless capability boundaries retain their own approvals.
