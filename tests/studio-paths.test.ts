...
import { checkedStudioPaths, studioPaths } from "../src/core/studio-paths.ts";
...
test("project paths are isolated, read-only, and V1 cannot collide with V2", async t => {
  ...
  const paths = checkedStudioPaths(cwd);
  ...
  assert.notEqual(studioPaths(join(cwd, "other")).root, paths.root);
  ...
  assert.deepEqual(checkedStudioPaths(cwd), paths);
...
for (const old of [".pi/knowledge-studio-v2", ".pi/knowledge-studio-v2-pdf-jobs", ".pi/knowledge-studio-v2-pdf-generations", "knowledge-studio-v2-exports", ".pi/knowledge-studio/collections/demo"]) {
  test(`legacy storage blocks without mutation: ${old}`, async t => {
    ...
    await mkdir(studioPaths(cwd).exports, { recursive: true, mode: 0o700 });
    assert.throws(() => checkedStudioPaths(cwd), /Legacy Studio storage detected.*no data was moved.*docs\/storage-layout.md/s);
    assert.equal(await readFile(join(source, "collection.json"), "utf8"), "original bytes");
    assert.deepEqual(await readdir(studioPaths(cwd).exports), []);
  });
}
...
test("quiescent V1 relocation preserves relative JSON store and symlink roots fail closed", async t => {
  ...
  assert.throws(() => checkedStudioPaths(cwd), /Legacy Studio storage/);
  ...
  assert.throws(() => checkedStudioPaths(cwd), /Unsafe Studio directory/);
});
